...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleAccessDenied(AccessDeniedException ex, HttpServletRequest req) {
        return response(HttpStatus.FORBIDDEN, ex.getMessage() != null ? ex.getMessage() : "Forbidden", req.getRequestURI());
    }
    ...
    public ResponseEntity<ApiError> handleBadRequestBody(HttpMessageNotReadableException ex, HttpServletRequest req) {
        ...
        if (ex.getMessage() != null && ex.getMessage().contains("UUID")) {
            message = "IDs must be valid UUIDs (e.g. 00000000-0000-0000-0000-000000000002). Got invalid value.";
    ...
    public ResponseEntity<ApiError> handleIllegalState(IllegalStateException ex, HttpServletRequest req) {
        if ("Missing user context".equals(ex.getMessage())) {
            return response(HttpStatus.UNAUTHORIZED, "Missing or invalid Authorization token (user context required)", req.getRequestURI());
        }
        return response(HttpStatus.BAD_REQUEST, ex.getMessage(), req.getRequestURI());
    }
